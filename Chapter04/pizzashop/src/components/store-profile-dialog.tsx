...
export const StoreProfileDialog = () => {
  ...
  function updateManagedRestaurantCache({ name, description }: StoreProfileSchema) {
    const cached = queryClient.getQueryData<{ data: GetManagedRestaurantResponse }>(['managed-restaurant'])
    ...
    if (cached) {
      ...
      queryClient.setQueryData<{ data: GetManagedRestaurantResponse }>(
        ['managed-restaurant'],
        newInfos,
      )
    }
  ...
  const { mutateAsync: updateProfileFn } = useMutation({
    ...
    onMutate({ name, description }) {
      ...
      return { previousProfile: cached?.data }
    },
